...
public class GameEngine {
	...
	public void coordinateRender(Tile[][] tiles) {
		final int CELL_WIDTH = 5;
		StringBuilder sb = new StringBuilder(MAP_WIDTH * MAP_HEIGHT * (CELL_WIDTH + 10));
		...
		for (int i = 0; i < MAP_HEIGHT; i++) {
			for (int j = 0; j < MAP_WIDTH; j++) {
				...
				if (tile.player1Present && tile.player2Present) {
					content = "3";
					color = AnsiColor.BOLD + AnsiColor.BRIGHT_YELLOW;
				} else if (tile.player1Present) {
					content = "1";
					color = AnsiColor.BOLD + AnsiColor.BRIGHT_CYAN;
				} else if (tile.player2Present) {
					content = "2";
					color = AnsiColor.BOLD + AnsiColor.BRIGHT_RED;
				} else if (tile.treasurePresent) {
					content = String.valueOf(tile.treasure.texture);
					color = (tile.treasure.value == 10) ? AnsiColor.YELLOW : AnsiColor.BRIGHT_GREEN;
				} else if (tile.texture == '@') {
					content = String.valueOf(tile.texture);
					...
					content = String.format("(%d,%d)", j, i);
					color = AnsiColor.WHITE;
				}

				// Pad content to fixed width
				if (content.length() < CELL_WIDTH) {
					content = String.format("%-" + CELL_WIDTH + "s", content);
				} else if (content.length() > CELL_WIDTH) {
					content = content.substring(0, CELL_WIDTH);
				}
	...
	public int getP1TeleportFlag() {
		return p1TeleportFlag;
	}

	public int getP2TeleportFlag() {
		return p2TeleportFlag;
	}
}
